# BASIC PROJECT USING *PYTHON*


In This Repository I Am Upload Only Basic Level Of Projects Using Python Language.


# 1 ->  Number Gussing Game .


A Python console game that challenges users to guess a randomly generated number *(randint)*, providing feedback on each attempt until the correct guess is made.


# 2 ->  Stone,Paper,Scissor Game .


A game using python with simple and easy rules.


Using more loops for check the condition (rules).


# 3 -> Password generator .

In this project  i uses simple level of python and random library (for randomly select any psasword from symbols,numbers,alphas)for making password generator .
